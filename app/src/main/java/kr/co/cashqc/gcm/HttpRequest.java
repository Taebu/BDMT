...
public class HttpRequest implements Runnable {
                                ...
                                for(int j=0;j<curNodeList.getLength();j++) {
                                    if(curNodeList.item(j).getNodeType()==Node.ELEMENT_NODE) {
                                        ...
                                        if(item.getFirstChild()!=null) {
                                            ...
                                            itemValue = itemValue.replaceAll("^[\\s　]*","").replaceAll("[\\s　]*$","");
			...
			if(conn!=null) {
				...
				if(conn.getResponseCode()==HttpURLConnection.HTTP_OK) {
					...
					if(topNodeList!=null && topNodeList.getLength()>0) {
						for(int i=0;i<topNodeList.getLength();i++) {
							Element topElement = (Element)topNodeList.item(i);
							Dictionary dic = getNodes(topElement);
							dicArray.add(dic);
						}
					}
				}
	...
	public Dictionary getNodes(Element item) {
		...
		if(childNodes != null && childNodes.getLength() > 0) {
			ArrayList<Dictionary> childs = new ArrayList<Dictionary>();
			for(int i=0;i<childNodes.getLength();i++) {
				Dictionary childDic = getNodes((Element)childNodes.item(i));
				childs.add(childDic);
			}
			dic.addObject(item.getNodeName(),childs);
			dic.setType(item.getNodeName(),Dictionary.DATA_ARRAYLIST_DIC);
		}
		else {
			getNodeValue(dic,item);
		}
		
		return dic;
	}
	
	public void getNodeValue(Dictionary dic, Element item) {
